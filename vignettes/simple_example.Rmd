...
```{r}
gapmap(m = as.matrix(distxy), d_row= rev(dend), d_col=dend,  mode = "quantitative", mapping="linear")
```
...
```{r}
gapmap(m = as.matrix(distxy), d_row= rev(dend), d_col=dend,  mode = "threshold", row_threshold = 2, col_threshold = 2)
```
...
```{r}
library(dendsort);
gapmap(m = as.matrix(distxy), d_row= dendsort(dend, isReverse=TRUE), d_col=dendsort(dend),  mode = "quantitative", mapping="linear")
```
